...
public class DriverMapActivity extends FragmentActivity implements OnMapReadyCallback, GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener, com.google.android.gms.location.LocationListener, RoutingListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Button mHistory = findViewById(R.id.history);
    ...
    private void getRouteToMarker(LatLng pickupLatLng) {
        Routing routing = new Routing.Builder().travelMode(AbstractRouting.TravelMode.DRIVING).withListener(this).alternativeRoutes(false).waypoints(new LatLng(mLastLocation.getLatitude(), mLastLocation.getLongitude()), pickupLatLng).build();
        routing.execute();
    }
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                if(dataSnapshot.exists() && dataSnapshot.getChildrenCount()>0){
                    ...
                    if (map != null && map.get("name") != null) {
                        mCustomerName.setText(Objects.requireNonNull(map.get("name")).toString());
                    }
                    if (map != null && map.get("phone") != null) {
                        mCustomerPhone.setText(Objects.requireNonNull(map.get("phone")).toString());
                    }
                    if (map != null && map.get("profileImageUrl") != null) {
                        Glide.with(getApplication()).load(Objects.requireNonNull(map.get("profileImageUrl")).toString()).into(mCustomerProfileImage);
                    }
                }
            }
            @Override
